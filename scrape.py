...
def main():
    ...
    # Write fresh data only when we actually have some — never overwrite the last
    # good JSON (draws + history the app relies on) with an empty result.
    if draws:
        # Accumulate per-category history (the sources expose only today's draw,
        # so history grows one run at a time). `draws` (latest) stays unchanged
        # for backward compat; `history` is additive — old app versions ignore it.
